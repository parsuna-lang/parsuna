//! Generated by parsuna — do not edit by hand.
//!
//! Pull-based, recoverable parser. Call one of the `parse_*_from_str` /
//! `parse_*_from_reader` constructors and iterate the resulting [`Parser`]
//! to walk the parse as a flat [`Event`] stream.
#![allow(dead_code, unused_imports)]

use std::io::Read;
use parsuna_rt::{DfaMatch, DfaMatcher, LexerBackend, Scanner, StreamingLexer, TERMINATED};

/// The set of token kinds this grammar can emit.
///
/// `Eof` marks end-of-input; the other variants come from the grammar's
/// `token` declarations. Lex failures (no pattern matched) are surfaced
/// as `Token { kind: None }` rather than an in-band error variant.
#[derive(Copy, Clone, Debug, PartialEq, Eq, Hash)]
#[repr(u16)]
pub enum TokenKind {
    Eof = 0,
    Lparen = 1,
    Rparen = 2,
    Lbrack = 3,
    Rbrack = 4,
    Comma = 5,
    Eq = 6,
    Semi = 7,
    Pipe = 8,
    Question = 9,
    Star = 10,
    Plus = 11,
    Dotdot = 12,
    Dot = 13,
    Bang = 14,
    String = 15,
    Char = 16,
    Ident = 17,
    Ws = 18,
    Comment = 19,
}

impl TokenKind {
    /// Numeric discriminant, equal to `self as u16`.
    pub const fn id(self) -> u16 { self as u16 }
}

impl parsuna_rt::TokenKindEnum for TokenKind {
    fn name(self) -> &'static str {
        match self {
            TokenKind::Eof => "EOF",
            TokenKind::Lparen => "LPAREN",
            TokenKind::Rparen => "RPAREN",
            TokenKind::Lbrack => "LBRACK",
            TokenKind::Rbrack => "RBRACK",
            TokenKind::Comma => "COMMA",
            TokenKind::Eq => "EQ",
            TokenKind::Semi => "SEMI",
            TokenKind::Pipe => "PIPE",
            TokenKind::Question => "QUESTION",
            TokenKind::Star => "STAR",
            TokenKind::Plus => "PLUS",
            TokenKind::Dotdot => "DOTDOT",
            TokenKind::Dot => "DOT",
            TokenKind::Bang => "BANG",
            TokenKind::String => "STRING",
            TokenKind::Char => "CHAR",
            TokenKind::Ident => "IDENT",
            TokenKind::Ws => "WS",
            TokenKind::Comment => "COMMENT",
        }
    }
    const EOF: Self = Self::Eof;
}

/// One variant per non-fragment rule in the grammar.
///
/// Attached to [`Event::Enter`] and [`Event::Exit`] so consumers can
/// tell which rule a subtree corresponds to.
#[derive(Copy, Clone, Debug, PartialEq, Eq, Hash)]
#[repr(u16)]
pub enum RuleKind {
    AltExpr = 0,
    Annot = 1,
    Annots = 2,
    Atom = 3,
    CharPrimary = 4,
    Decl = 5,
    File = 6,
    Group = 7,
    NegClass = 8,
    SeqExpr = 9,
}

impl RuleKind {
    /// Numeric discriminant, equal to `self as u16`.
    pub const fn id(self) -> u16 { self as u16 }
}

impl parsuna_rt::RuleKindEnum for RuleKind {
    fn name(self) -> &'static str {
        match self {
            RuleKind::AltExpr => "alt_expr",
            RuleKind::Annot => "annot",
            RuleKind::Annots => "annots",
            RuleKind::Atom => "atom",
            RuleKind::CharPrimary => "char_primary",
            RuleKind::Decl => "decl",
            RuleKind::File => "file",
            RuleKind::Group => "group",
            RuleKind::NegClass => "neg_class",
            RuleKind::SeqExpr => "seq_expr",
        }
    }
}



/// Re-exports of the runtime types used by this parser.
pub use parsuna_rt::{Span, Pos, Error};
/// Parse event carrying this grammar's token and rule kinds.
pub type Event<'a> = parsuna_rt::Event<'a, TokenKind, RuleKind>;
/// A lexed token with this grammar's [`TokenKind`].
pub type Token<'a> = parsuna_rt::Token<'a, TokenKind>;


/// Compiled lexer DFA for this grammar.
///
/// Zero-sized type whose [`DfaMatcher`] impl drives the lexer via a
/// per-state `match` rather than a transition table. Generated code
/// supplies this to [`Scanner`]/[`StreamingLexer`].
pub struct LexerDfa;

impl DfaMatcher<TokenKind> for LexerDfa {
    #[inline]
    fn longest_match(buf: &[u8], start: usize) -> DfaMatch<TokenKind> {
        let mut pos = start;
        let mut best_len: usize = 0;
        let mut best_kind: Option<TokenKind> = None;
        let mut state: u32 = 1;
        loop {
            match state {
                1 => match buf.get(pos) {
                    Some(&b'\t') => { pos += 1; state = 2; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\n') => { pos += 1; state = 3; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\r') => { pos += 1; state = 4; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b' ') => { pos += 1; state = 5; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'!') => { pos += 1; state = 6; best_len = pos - start; best_kind = Some(TokenKind::Bang); }
                    Some(&b'"') => { pos += 1; state = 7; }
                    Some(&b'\'') => { pos += 1; state = 8; }
                    Some(&b'(') => { pos += 1; state = 9; best_len = pos - start; best_kind = Some(TokenKind::Lparen); }
                    Some(&b')') => { pos += 1; state = 10; best_len = pos - start; best_kind = Some(TokenKind::Rparen); }
                    Some(&b'*') => { pos += 1; state = 11; best_len = pos - start; best_kind = Some(TokenKind::Star); }
                    Some(&b'+') => { pos += 1; state = 12; best_len = pos - start; best_kind = Some(TokenKind::Plus); }
                    Some(&b',') => { pos += 1; state = 13; best_len = pos - start; best_kind = Some(TokenKind::Comma); }
                    Some(&b'.') => { pos += 1; state = 14; best_len = pos - start; best_kind = Some(TokenKind::Dot); }
                    Some(&b'/') => { pos += 1; state = 15; }
                    Some(&b';') => { pos += 1; state = 16; best_len = pos - start; best_kind = Some(TokenKind::Semi); }
                    Some(&b'=') => { pos += 1; state = 17; best_len = pos - start; best_kind = Some(TokenKind::Eq); }
                    Some(&b'?') => { pos += 1; state = 18; best_len = pos - start; best_kind = Some(TokenKind::Question); }
                    Some(&(b'A'..=b'Z')) => { pos += 1; state = 19; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&b'[') => { pos += 1; state = 20; best_len = pos - start; best_kind = Some(TokenKind::Lbrack); }
                    Some(&b']') => { pos += 1; state = 21; best_len = pos - start; best_kind = Some(TokenKind::Rbrack); }
                    Some(&b'_') => { pos += 1; state = 22; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'a'..=b'z')) => { pos += 1; state = 23; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&b'|') => { pos += 1; state = 24; best_len = pos - start; best_kind = Some(TokenKind::Pipe); }
                    _ => break,
                },
                2 => match buf.get(pos) {
                    Some(&b'\t') => { pos += 1; state = 25; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\n') => { pos += 1; state = 26; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\r') => { pos += 1; state = 27; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b' ') => { pos += 1; state = 28; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    _ => break,
                },
                3 => match buf.get(pos) {
                    Some(&b'\t') => { pos += 1; state = 25; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\n') => { pos += 1; state = 26; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\r') => { pos += 1; state = 27; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b' ') => { pos += 1; state = 28; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    _ => break,
                },
                4 => match buf.get(pos) {
                    Some(&b'\t') => { pos += 1; state = 25; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\n') => { pos += 1; state = 26; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\r') => { pos += 1; state = 27; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b' ') => { pos += 1; state = 28; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    _ => break,
                },
                5 => match buf.get(pos) {
                    Some(&b'\t') => { pos += 1; state = 25; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\n') => { pos += 1; state = 26; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\r') => { pos += 1; state = 27; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b' ') => { pos += 1; state = 28; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    _ => break,
                },
                6 => break,
                7 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                8 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'&' | b'('..=b'[' | b']'..=0x7f)) => { pos += 1; state = 40; }
                    Some(&b'\\') => { pos += 1; state = 41; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 42; }
                    Some(&0xe0) => { pos += 1; state = 43; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 44; }
                    Some(&0xed) => { pos += 1; state = 45; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 46; }
                    Some(&0xf0) => { pos += 1; state = 47; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 48; }
                    Some(&0xf4) => { pos += 1; state = 49; }
                    _ => break,
                },
                9 => break,
                10 => break,
                11 => break,
                12 => break,
                13 => break,
                14 => match buf.get(pos) {
                    Some(&b'.') => { pos += 1; state = 50; best_len = pos - start; best_kind = Some(TokenKind::Dotdot); }
                    _ => break,
                },
                15 => match buf.get(pos) {
                    Some(&b'/') => { pos += 1; state = 51; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    _ => break,
                },
                16 => break,
                17 => break,
                18 => break,
                19 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 52; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'A'..=b'Z')) => { pos += 1; state = 53; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&b'_') => { pos += 1; state = 54; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'a'..=b'z')) => { pos += 1; state = 55; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    _ => break,
                },
                20 => break,
                21 => break,
                22 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 52; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'A'..=b'Z')) => { pos += 1; state = 53; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&b'_') => { pos += 1; state = 54; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'a'..=b'z')) => { pos += 1; state = 55; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    _ => break,
                },
                23 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 52; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'A'..=b'Z')) => { pos += 1; state = 53; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&b'_') => { pos += 1; state = 54; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'a'..=b'z')) => { pos += 1; state = 55; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    _ => break,
                },
                24 => break,
                25 => match buf.get(pos) {
                    Some(&b'\t') => { pos += 1; state = 25; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\n') => { pos += 1; state = 26; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\r') => { pos += 1; state = 27; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b' ') => { pos += 1; state = 28; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    _ => break,
                },
                26 => match buf.get(pos) {
                    Some(&b'\t') => { pos += 1; state = 25; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\n') => { pos += 1; state = 26; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\r') => { pos += 1; state = 27; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b' ') => { pos += 1; state = 28; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    _ => break,
                },
                27 => match buf.get(pos) {
                    Some(&b'\t') => { pos += 1; state = 25; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\n') => { pos += 1; state = 26; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\r') => { pos += 1; state = 27; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b' ') => { pos += 1; state = 28; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    _ => break,
                },
                28 => match buf.get(pos) {
                    Some(&b'\t') => { pos += 1; state = 25; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\n') => { pos += 1; state = 26; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b'\r') => { pos += 1; state = 27; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    Some(&b' ') => { pos += 1; state = 28; best_len = pos - start; best_kind = Some(TokenKind::Ws); }
                    _ => break,
                },
                29 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                30 => break,
                31 => match buf.get(pos) {
                    Some(&b'"') => { pos += 1; state = 56; }
                    Some(&b'\'') => { pos += 1; state = 57; }
                    Some(&b'0') => { pos += 1; state = 58; }
                    Some(&b'\\') => { pos += 1; state = 59; }
                    Some(&b'n') => { pos += 1; state = 60; }
                    Some(&b'r') => { pos += 1; state = 61; }
                    Some(&b't') => { pos += 1; state = 62; }
                    Some(&b'u') => { pos += 1; state = 63; }
                    _ => break,
                },
                32 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 29; }
                    _ => break,
                },
                33 => match buf.get(pos) {
                    Some(&(0xa0..=0xbf)) => { pos += 1; state = 64; }
                    _ => break,
                },
                34 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 65; }
                    _ => break,
                },
                35 => match buf.get(pos) {
                    Some(&(0x80..=0x9f)) => { pos += 1; state = 66; }
                    _ => break,
                },
                36 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 67; }
                    _ => break,
                },
                37 => match buf.get(pos) {
                    Some(&(0x90..=0xbf)) => { pos += 1; state = 68; }
                    _ => break,
                },
                38 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 69; }
                    _ => break,
                },
                39 => match buf.get(pos) {
                    Some(&(0x80..=0x8f)) => { pos += 1; state = 70; }
                    _ => break,
                },
                40 => match buf.get(pos) {
                    Some(&b'\'') => { pos += 1; state = 71; best_len = pos - start; best_kind = Some(TokenKind::Char); }
                    _ => break,
                },
                41 => match buf.get(pos) {
                    Some(&b'"') => { pos += 1; state = 72; }
                    Some(&b'\'') => { pos += 1; state = 73; }
                    Some(&b'0') => { pos += 1; state = 74; }
                    Some(&b'\\') => { pos += 1; state = 75; }
                    Some(&b'n') => { pos += 1; state = 76; }
                    Some(&b'r') => { pos += 1; state = 77; }
                    Some(&b't') => { pos += 1; state = 78; }
                    Some(&b'u') => { pos += 1; state = 79; }
                    _ => break,
                },
                42 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 40; }
                    _ => break,
                },
                43 => match buf.get(pos) {
                    Some(&(0xa0..=0xbf)) => { pos += 1; state = 80; }
                    _ => break,
                },
                44 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 81; }
                    _ => break,
                },
                45 => match buf.get(pos) {
                    Some(&(0x80..=0x9f)) => { pos += 1; state = 82; }
                    _ => break,
                },
                46 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 83; }
                    _ => break,
                },
                47 => match buf.get(pos) {
                    Some(&(0x90..=0xbf)) => { pos += 1; state = 84; }
                    _ => break,
                },
                48 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 85; }
                    _ => break,
                },
                49 => match buf.get(pos) {
                    Some(&(0x80..=0x8f)) => { pos += 1; state = 86; }
                    _ => break,
                },
                50 => break,
                51 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=0x7f)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 88; }
                    Some(&0xe0) => { pos += 1; state = 89; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 90; }
                    Some(&0xed) => { pos += 1; state = 91; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 92; }
                    Some(&0xf0) => { pos += 1; state = 93; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 94; }
                    Some(&0xf4) => { pos += 1; state = 95; }
                    _ => break,
                },
                52 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 52; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'A'..=b'Z')) => { pos += 1; state = 53; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&b'_') => { pos += 1; state = 54; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'a'..=b'z')) => { pos += 1; state = 55; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    _ => break,
                },
                53 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 52; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'A'..=b'Z')) => { pos += 1; state = 53; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&b'_') => { pos += 1; state = 54; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'a'..=b'z')) => { pos += 1; state = 55; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    _ => break,
                },
                54 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 52; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'A'..=b'Z')) => { pos += 1; state = 53; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&b'_') => { pos += 1; state = 54; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'a'..=b'z')) => { pos += 1; state = 55; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    _ => break,
                },
                55 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 52; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'A'..=b'Z')) => { pos += 1; state = 53; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&b'_') => { pos += 1; state = 54; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    Some(&(b'a'..=b'z')) => { pos += 1; state = 55; best_len = pos - start; best_kind = Some(TokenKind::Ident); }
                    _ => break,
                },
                56 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                57 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                58 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                59 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                60 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                61 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                62 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                63 => match buf.get(pos) {
                    Some(&b'{') => { pos += 1; state = 96; }
                    _ => break,
                },
                64 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 29; }
                    _ => break,
                },
                65 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 29; }
                    _ => break,
                },
                66 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 29; }
                    _ => break,
                },
                67 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 29; }
                    _ => break,
                },
                68 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 97; }
                    _ => break,
                },
                69 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 98; }
                    _ => break,
                },
                70 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 99; }
                    _ => break,
                },
                71 => break,
                72 => match buf.get(pos) {
                    Some(&b'\'') => { pos += 1; state = 71; best_len = pos - start; best_kind = Some(TokenKind::Char); }
                    _ => break,
                },
                73 => match buf.get(pos) {
                    Some(&b'\'') => { pos += 1; state = 71; best_len = pos - start; best_kind = Some(TokenKind::Char); }
                    _ => break,
                },
                74 => match buf.get(pos) {
                    Some(&b'\'') => { pos += 1; state = 71; best_len = pos - start; best_kind = Some(TokenKind::Char); }
                    _ => break,
                },
                75 => match buf.get(pos) {
                    Some(&b'\'') => { pos += 1; state = 71; best_len = pos - start; best_kind = Some(TokenKind::Char); }
                    _ => break,
                },
                76 => match buf.get(pos) {
                    Some(&b'\'') => { pos += 1; state = 71; best_len = pos - start; best_kind = Some(TokenKind::Char); }
                    _ => break,
                },
                77 => match buf.get(pos) {
                    Some(&b'\'') => { pos += 1; state = 71; best_len = pos - start; best_kind = Some(TokenKind::Char); }
                    _ => break,
                },
                78 => match buf.get(pos) {
                    Some(&b'\'') => { pos += 1; state = 71; best_len = pos - start; best_kind = Some(TokenKind::Char); }
                    _ => break,
                },
                79 => match buf.get(pos) {
                    Some(&b'{') => { pos += 1; state = 100; }
                    _ => break,
                },
                80 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 40; }
                    _ => break,
                },
                81 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 40; }
                    _ => break,
                },
                82 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 40; }
                    _ => break,
                },
                83 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 40; }
                    _ => break,
                },
                84 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 101; }
                    _ => break,
                },
                85 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 102; }
                    _ => break,
                },
                86 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 103; }
                    _ => break,
                },
                87 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=0x7f)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 88; }
                    Some(&0xe0) => { pos += 1; state = 89; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 90; }
                    Some(&0xed) => { pos += 1; state = 91; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 92; }
                    Some(&0xf0) => { pos += 1; state = 93; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 94; }
                    Some(&0xf4) => { pos += 1; state = 95; }
                    _ => break,
                },
                88 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    _ => break,
                },
                89 => match buf.get(pos) {
                    Some(&(0xa0..=0xbf)) => { pos += 1; state = 104; }
                    _ => break,
                },
                90 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 105; }
                    _ => break,
                },
                91 => match buf.get(pos) {
                    Some(&(0x80..=0x9f)) => { pos += 1; state = 106; }
                    _ => break,
                },
                92 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 107; }
                    _ => break,
                },
                93 => match buf.get(pos) {
                    Some(&(0x90..=0xbf)) => { pos += 1; state = 108; }
                    _ => break,
                },
                94 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 109; }
                    _ => break,
                },
                95 => match buf.get(pos) {
                    Some(&(0x80..=0x8f)) => { pos += 1; state = 110; }
                    _ => break,
                },
                96 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 111; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 112; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 113; }
                    _ => break,
                },
                97 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 29; }
                    _ => break,
                },
                98 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 29; }
                    _ => break,
                },
                99 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 29; }
                    _ => break,
                },
                100 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 114; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 115; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 116; }
                    _ => break,
                },
                101 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 40; }
                    _ => break,
                },
                102 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 40; }
                    _ => break,
                },
                103 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 40; }
                    _ => break,
                },
                104 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    _ => break,
                },
                105 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    _ => break,
                },
                106 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    _ => break,
                },
                107 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    _ => break,
                },
                108 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 117; }
                    _ => break,
                },
                109 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 118; }
                    _ => break,
                },
                110 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 119; }
                    _ => break,
                },
                111 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 120; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 121; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 122; }
                    Some(&b'}') => { pos += 1; state = 123; }
                    _ => break,
                },
                112 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 120; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 121; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 122; }
                    Some(&b'}') => { pos += 1; state = 123; }
                    _ => break,
                },
                113 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 120; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 121; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 122; }
                    Some(&b'}') => { pos += 1; state = 123; }
                    _ => break,
                },
                114 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 124; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 125; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 126; }
                    Some(&b'}') => { pos += 1; state = 127; }
                    _ => break,
                },
                115 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 124; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 125; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 126; }
                    Some(&b'}') => { pos += 1; state = 127; }
                    _ => break,
                },
                116 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 124; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 125; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 126; }
                    Some(&b'}') => { pos += 1; state = 127; }
                    _ => break,
                },
                117 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    _ => break,
                },
                118 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    _ => break,
                },
                119 => match buf.get(pos) {
                    Some(&(0x80..=0xbf)) => { pos += 1; state = 87; best_len = pos - start; best_kind = Some(TokenKind::Comment); }
                    _ => break,
                },
                120 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 120; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 121; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 122; }
                    Some(&b'}') => { pos += 1; state = 123; }
                    _ => break,
                },
                121 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 120; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 121; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 122; }
                    Some(&b'}') => { pos += 1; state = 123; }
                    _ => break,
                },
                122 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 120; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 121; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 122; }
                    Some(&b'}') => { pos += 1; state = 123; }
                    _ => break,
                },
                123 => match buf.get(pos) {
                    Some(&(0x00..=b'\t' | 0x0b..=b'!' | b'#'..=b'[' | b']'..=0x7f)) => { pos += 1; state = 29; }
                    Some(&b'"') => { pos += 1; state = 30; best_len = pos - start; best_kind = Some(TokenKind::String); }
                    Some(&b'\\') => { pos += 1; state = 31; }
                    Some(&(0xc2..=0xdf)) => { pos += 1; state = 32; }
                    Some(&0xe0) => { pos += 1; state = 33; }
                    Some(&(0xe1..=0xec)) => { pos += 1; state = 34; }
                    Some(&0xed) => { pos += 1; state = 35; }
                    Some(&(0xee..=0xef)) => { pos += 1; state = 36; }
                    Some(&0xf0) => { pos += 1; state = 37; }
                    Some(&(0xf1..=0xf3)) => { pos += 1; state = 38; }
                    Some(&0xf4) => { pos += 1; state = 39; }
                    _ => break,
                },
                124 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 124; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 125; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 126; }
                    Some(&b'}') => { pos += 1; state = 127; }
                    _ => break,
                },
                125 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 124; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 125; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 126; }
                    Some(&b'}') => { pos += 1; state = 127; }
                    _ => break,
                },
                126 => match buf.get(pos) {
                    Some(&(b'0'..=b'9')) => { pos += 1; state = 124; }
                    Some(&(b'A'..=b'F')) => { pos += 1; state = 125; }
                    Some(&(b'a'..=b'f')) => { pos += 1; state = 126; }
                    Some(&b'}') => { pos += 1; state = 127; }
                    _ => break,
                },
                127 => match buf.get(pos) {
                    Some(&b'\'') => { pos += 1; state = 71; best_len = pos - start; best_kind = Some(TokenKind::Char); }
                    _ => break,
                },
                _ => break,
            }
        }
        DfaMatch { best_len, best_kind, scanned: pos - start }
    }
}


/// Lookahead required to disambiguate every alternative (LL(k)).
pub const K: usize = 1;
const ENTRY_FILE: u32 = 1;
const ENTRY_DECL: u32 = 7;
const ENTRY_ANNOTS: u32 = 17;
const ENTRY_ANNOT: u32 = 27;
const ENTRY_ALT_EXPR: u32 = 31;
const ENTRY_SEQ_EXPR: u32 = 39;
const ENTRY_GROUP: u32 = 64;
const ENTRY_ATOM: u32 = 70;
const ENTRY_CHAR_PRIMARY: u32 = 82;
const ENTRY_NEG_CLASS: u32 = 94;

static FIRST_0: &[&[TokenKind]] = &[&[TokenKind::Ident]];
static FIRST_1: &[&[TokenKind]] = &[&[TokenKind::Lbrack]];
static FIRST_2: &[&[TokenKind]] = &[&[TokenKind::Comma]];
static FIRST_3: &[&[TokenKind]] = &[&[TokenKind::Pipe]];
static FIRST_4: &[&[TokenKind]] = &[&[TokenKind::Lparen], &[TokenKind::Dot], &[TokenKind::Bang], &[TokenKind::String], &[TokenKind::Char], &[TokenKind::Ident]];
static FIRST_5: &[&[TokenKind]] = &[&[TokenKind::Question], &[TokenKind::Star], &[TokenKind::Plus]];
static FIRST_6: &[&[TokenKind]] = &[&[TokenKind::Question]];
static FIRST_7: &[&[TokenKind]] = &[&[TokenKind::Star]];
static FIRST_8: &[&[TokenKind]] = &[&[TokenKind::Plus]];
static FIRST_9: &[&[TokenKind]] = &[&[TokenKind::Lparen]];
static FIRST_10: &[&[TokenKind]] = &[&[TokenKind::Dot], &[TokenKind::Bang], &[TokenKind::String], &[TokenKind::Char], &[TokenKind::Ident]];
static FIRST_11: &[&[TokenKind]] = &[&[TokenKind::String]];
static FIRST_12: &[&[TokenKind]] = &[&[TokenKind::Dot], &[TokenKind::Char]];
static FIRST_13: &[&[TokenKind]] = &[&[TokenKind::Bang]];
static FIRST_14: &[&[TokenKind]] = &[&[TokenKind::Char]];
static FIRST_15: &[&[TokenKind]] = &[&[TokenKind::Dotdot]];
static FIRST_16: &[&[TokenKind]] = &[&[TokenKind::Dot]];

static SYNC_0: &[TokenKind] = &[TokenKind::Eof];
static SYNC_1: &[TokenKind] = &[TokenKind::Eof, TokenKind::Ident];
static SYNC_2: &[TokenKind] = &[TokenKind::Eof, TokenKind::Rbrack, TokenKind::Comma, TokenKind::Ident];
static SYNC_3: &[TokenKind] = &[TokenKind::Eof, TokenKind::Lparen, TokenKind::Rparen, TokenKind::Lbrack, TokenKind::Semi, TokenKind::Pipe, TokenKind::Question, TokenKind::Star, TokenKind::Plus, TokenKind::Dot, TokenKind::Bang, TokenKind::String, TokenKind::Char, TokenKind::Ident];


/// Zero-sized marker type that carries the generated dispatch logic via
/// [`parsuna_rt::Drive`]. You never construct one directly — it's a
/// type-level parameter to [`Parser`].
pub struct Grammar;

/// Parser alias pinning the grammar and lookahead. `L` is any
/// [`LexerBackend`]; the generated `parse_*_from_str`/`parse_*_from_reader`
/// helpers build a parser with either [`Scanner`] or [`StreamingLexer`].
pub type Parser<'a, L> = parsuna_rt::Parser<'a, L, K, Grammar>;
impl parsuna_rt::Drive<K> for Grammar {
    type TokenKind = TokenKind;
    type RuleKind = RuleKind;
    const HAS_SKIPS: bool = true;

    #[inline(always)]
    fn is_skip(kind: TokenKind) -> bool {
        matches!(kind, TokenKind::Ws | TokenKind::Comment)
    }

    #[inline]
    fn drive<'a, L: LexerBackend<'a, Self::TokenKind>>(p: &mut parsuna_rt::Parser<'a, L, K, Self>) {
        let mut cur = p.state();
        while p.queue_is_empty() && cur != TERMINATED {
            match cur {
                1 => { // file:enter
                    p.enter(RuleKind::File);
                    cur = 2;
                }
                2 => { // file:star
                    match p.look(0).kind {
                        Some(TokenKind::Ident) => {
                            p.push_ret(2); cur = 5;
                        }
                        _ => {
                            cur = 3;
                        }
                    }
                }
                3 => { // file:exit
                    p.exit(RuleKind::File);
                    cur = p.ret();
                }
                5 => { // file:star-body:call:decl
                    p.push_ret(6);
                    p.enter(RuleKind::Decl);
                    p.expect(TokenKind::Ident, SYNC_1, "expected IDENT");
                    p.expect(TokenKind::Eq, SYNC_1, "expected EQ");
                    p.push_ret(11);
                    p.enter(RuleKind::AltExpr);
                    p.push_ret(33);
                    cur = 39;
                }
                6 => { // file:star-body:ret
                    cur = p.ret();
                }
                7 => { // decl:enter
                    p.enter(RuleKind::Decl);
                    p.expect(TokenKind::Ident, SYNC_1, "expected IDENT");
                    p.expect(TokenKind::Eq, SYNC_1, "expected EQ");
                    p.push_ret(11);
                    p.enter(RuleKind::AltExpr);
                    p.push_ret(33);
                    p.enter(RuleKind::SeqExpr);
                    cur = 40;
                }
                11 => { // decl:opt
                    match p.look(0).kind {
                        Some(TokenKind::Lbrack) => {
                            p.push_ret(12); cur = 15;
                        }
                        _ => {
                            cur = 12;
                        }
                    }
                }
                12 => { // decl:expect:SEMI
                    p.expect(TokenKind::Semi, SYNC_1, "expected SEMI");
                    p.exit(RuleKind::Decl);
                    cur = p.ret();
                }
                15 => { // decl:opt-body:call:annots
                    p.push_ret(16);
                    p.enter(RuleKind::Annots);
                    p.expect(TokenKind::Lbrack, SYNC_1, "expected LBRACK");
                    p.push_ret(20);
                    p.enter(RuleKind::Annot);
                    p.expect(TokenKind::Ident, SYNC_2, "expected IDENT");
                    p.exit(RuleKind::Annot);
                    cur = 30;
                }
                16 => { // decl:opt-body:ret
                    cur = p.ret();
                }
                17 => { // annots:enter
                    p.enter(RuleKind::Annots);
                    p.expect(TokenKind::Lbrack, SYNC_1, "expected LBRACK");
                    p.push_ret(20);
                    p.enter(RuleKind::Annot);
                    p.expect(TokenKind::Ident, SYNC_2, "expected IDENT");
                    p.exit(RuleKind::Annot);
                    cur = p.ret();
                }
                20 => { // annots:star
                    match p.look(0).kind {
                        Some(TokenKind::Comma) => {
                            p.push_ret(20); cur = 24;
                        }
                        _ => {
                            cur = 21;
                        }
                    }
                }
                21 => { // annots:expect:RBRACK
                    p.expect(TokenKind::Rbrack, SYNC_1, "expected RBRACK");
                    p.exit(RuleKind::Annots);
                    cur = p.ret();
                }
                24 => { // annots:star-body:expect:COMMA
                    p.expect(TokenKind::Comma, SYNC_1, "expected COMMA");
                    p.push_ret(26);
                    p.enter(RuleKind::Annot);
                    p.expect(TokenKind::Ident, SYNC_2, "expected IDENT");
                    p.exit(RuleKind::Annot);
                    cur = p.ret();
                }
                26 => { // annots:star-body:ret
                    cur = p.ret();
                }
                27 => { // annot:enter
                    p.enter(RuleKind::Annot);
                    p.expect(TokenKind::Ident, SYNC_2, "expected IDENT");
                    p.exit(RuleKind::Annot);
                    cur = p.ret();
                }
                30 => { // annot:ret
                    cur = p.ret();
                }
                31 => { // alt_expr:enter
                    p.enter(RuleKind::AltExpr);
                    p.push_ret(33);
                    p.enter(RuleKind::SeqExpr);
                    cur = 40;
                }
                33 => { // alt_expr:star
                    match p.look(0).kind {
                        Some(TokenKind::Pipe) => {
                            p.push_ret(33); cur = 36;
                        }
                        _ => {
                            cur = 34;
                        }
                    }
                }
                34 => { // alt_expr:exit
                    p.exit(RuleKind::AltExpr);
                    cur = p.ret();
                }
                36 => { // alt_expr:star-body:expect:PIPE
                    p.expect(TokenKind::Pipe, SYNC_3, "expected PIPE");
                    p.push_ret(38);
                    p.enter(RuleKind::SeqExpr);
                    cur = 40;
                }
                38 => { // alt_expr:star-body:ret
                    cur = p.ret();
                }
                39 => { // seq_expr:enter
                    p.enter(RuleKind::SeqExpr);
                    cur = 40;
                }
                40 => { // seq_expr:star
                    match p.look(0).kind {
                        Some(TokenKind::Lparen) | Some(TokenKind::Dot) | Some(TokenKind::Bang) | Some(TokenKind::String) | Some(TokenKind::Char) | Some(TokenKind::Ident) => {
                            p.push_ret(40); cur = 43;
                        }
                        _ => {
                            cur = 41;
                        }
                    }
                }
                41 => { // seq_expr:exit
                    p.exit(RuleKind::SeqExpr);
                    cur = p.ret();
                }
                43 => { // seq_expr:star-body:call:_postfix_expr
                    p.push_ret(44);
                    p.push_ret(46);
                    cur = 58;
                }
                44 => { // seq_expr:star-body:ret
                    cur = p.ret();
                }
                46 => { // _postfix_expr:star
                    match p.look(0).kind {
                        Some(TokenKind::Question) | Some(TokenKind::Star) | Some(TokenKind::Plus) => {
                            p.push_ret(46); cur = 48;
                        }
                        _ => {
                            cur = 47;
                        }
                    }
                }
                47 => { // _postfix_expr:ret
                    cur = p.ret();
                }
                48 => { // _postfix_expr:star-body:call:_quant_op
                    p.push_ret(49);
                    cur = 50;
                }
                49 => { // _postfix_expr:star-body:ret
                    cur = p.ret();
                }
                50 => { // _quant_op:dispatch
                    match p.look(0).kind {
                        Some(TokenKind::Question) => { p.push_ret(51); cur = 52; }
                        Some(TokenKind::Star) => { p.push_ret(51); cur = 54; }
                        Some(TokenKind::Plus) => { p.push_ret(51); cur = 56; }
                        _ => { cur = 51; p.error_here("unexpected token"); p.recover_to(SYNC_3); }
                    }
                }
                51 => { // _quant_op:ret
                    cur = p.ret();
                }
                52 => { // _quant_op:alt0:expect:QUESTION
                    p.expect(TokenKind::Question, SYNC_3, "expected QUESTION");
                    cur = p.ret();
                }
                54 => { // _quant_op:alt1:expect:STAR
                    p.expect(TokenKind::Star, SYNC_3, "expected STAR");
                    cur = p.ret();
                }
                56 => { // _quant_op:alt2:expect:PLUS
                    p.expect(TokenKind::Plus, SYNC_3, "expected PLUS");
                    cur = p.ret();
                }
                58 => { // _primary_expr:dispatch
                    match p.look(0).kind {
                        Some(TokenKind::Lparen) => { p.push_ret(59); cur = 60; }
                        Some(TokenKind::Dot) => { p.push_ret(59); cur = 62; }
                        Some(TokenKind::Bang) => { p.push_ret(59); cur = 62; }
                        Some(TokenKind::String) => { p.push_ret(59); cur = 62; }
                        Some(TokenKind::Char) => { p.push_ret(59); cur = 62; }
                        Some(TokenKind::Ident) => { p.push_ret(59); cur = 62; }
                        _ => { cur = 59; p.error_here("unexpected token"); p.recover_to(SYNC_3); }
                    }
                }
                59 => { // _primary_expr:ret
                    cur = p.ret();
                }
                60 => { // _primary_expr:alt0:call:group
                    p.push_ret(61);
                    p.enter(RuleKind::Group);
                    p.expect(TokenKind::Lparen, SYNC_3, "expected LPAREN");
                    p.push_ret(67);
                    p.enter(RuleKind::AltExpr);
                    p.push_ret(33);
                    p.enter(RuleKind::SeqExpr);
                    cur = 40;
                }
                61 => { // _primary_expr:alt0:ret
                    cur = p.ret();
                }
                62 => { // _primary_expr:alt1:call:atom
                    p.push_ret(63);
                    p.enter(RuleKind::Atom);
                    cur = 71;
                }
                63 => { // _primary_expr:alt1:ret
                    cur = p.ret();
                }
                64 => { // group:enter
                    p.enter(RuleKind::Group);
                    p.expect(TokenKind::Lparen, SYNC_3, "expected LPAREN");
                    p.push_ret(67);
                    p.enter(RuleKind::AltExpr);
                    p.push_ret(33);
                    p.enter(RuleKind::SeqExpr);
                    cur = 40;
                }
                67 => { // group:expect:RPAREN
                    p.expect(TokenKind::Rparen, SYNC_3, "expected RPAREN");
                    p.exit(RuleKind::Group);
                    cur = p.ret();
                }
                70 => { // atom:enter
                    p.enter(RuleKind::Atom);
                    cur = 71;
                }
                71 => { // atom:dispatch
                    match p.look(0).kind {
                        Some(TokenKind::Dot) => { p.push_ret(72); cur = 78; }
                        Some(TokenKind::Bang) => { p.push_ret(72); cur = 80; }
                        Some(TokenKind::String) => { p.push_ret(72); cur = 76; }
                        Some(TokenKind::Char) => { p.push_ret(72); cur = 78; }
                        Some(TokenKind::Ident) => { p.push_ret(72); cur = 74; }
                        _ => { cur = 72; p.error_here("unexpected token"); p.recover_to(SYNC_3); }
                    }
                }
                72 => { // atom:exit
                    p.exit(RuleKind::Atom);
                    cur = p.ret();
                }
                74 => { // atom:alt0:expect:IDENT
                    p.expect(TokenKind::Ident, SYNC_3, "expected IDENT");
                    cur = p.ret();
                }
                76 => { // atom:alt1:expect:STRING
                    p.expect(TokenKind::String, SYNC_3, "expected STRING");
                    cur = p.ret();
                }
                78 => { // atom:alt2:call:char_primary
                    p.push_ret(79);
                    p.enter(RuleKind::CharPrimary);
                    cur = 83;
                }
                79 => { // atom:alt2:ret
                    cur = p.ret();
                }
                80 => { // atom:alt3:call:neg_class
                    p.push_ret(81);
                    p.enter(RuleKind::NegClass);
                    p.expect(TokenKind::Bang, SYNC_3, "expected BANG");
                    cur = 96;
                }
                81 => { // atom:alt3:ret
                    cur = p.ret();
                }
                82 => { // char_primary:enter
                    p.enter(RuleKind::CharPrimary);
                    cur = 83;
                }
                83 => { // char_primary:dispatch
                    match p.look(0).kind {
                        Some(TokenKind::Dot) => { p.push_ret(84); cur = 89; }
                        Some(TokenKind::Char) => { p.push_ret(84); cur = 86; }
                        _ => { cur = 84; p.error_here("unexpected token"); p.recover_to(SYNC_3); }
                    }
                }
                84 => { // char_primary:exit
                    p.exit(RuleKind::CharPrimary);
                    cur = p.ret();
                }
                86 => { // char_primary:alt0:expect:CHAR
                    p.expect(TokenKind::Char, SYNC_3, "expected CHAR");
                    cur = 87;
                }
                87 => { // char_primary:alt0:opt
                    match p.look(0).kind {
                        Some(TokenKind::Dotdot) => {
                            p.push_ret(88); cur = 91;
                        }
                        _ => {
                            cur = 88;
                        }
                    }
                }
                88 => { // char_primary:alt0:ret
                    cur = p.ret();
                }
                89 => { // char_primary:alt1:expect:DOT
                    p.expect(TokenKind::Dot, SYNC_3, "expected DOT");
                    cur = p.ret();
                }
                91 => { // char_primary:alt0:opt-body:expect:DOTDOT
                    p.expect(TokenKind::Dotdot, SYNC_3, "expected DOTDOT");
                    p.expect(TokenKind::Char, SYNC_3, "expected CHAR");
                    cur = p.ret();
                }
                94 => { // neg_class:enter
                    p.enter(RuleKind::NegClass);
                    p.expect(TokenKind::Bang, SYNC_3, "expected BANG");
                    cur = 96;
                }
                96 => { // neg_class:dispatch
                    match p.look(0).kind {
                        Some(TokenKind::Lparen) => { p.push_ret(97); cur = 101; }
                        Some(TokenKind::Dot) => { p.push_ret(97); cur = 99; }
                        Some(TokenKind::Char) => { p.push_ret(97); cur = 99; }
                        _ => { cur = 97; p.error_here("unexpected token"); p.recover_to(SYNC_3); }
                    }
                }
                97 => { // neg_class:exit
                    p.exit(RuleKind::NegClass);
                    cur = p.ret();
                }
                99 => { // neg_class:alt0:call:char_primary
                    p.push_ret(100);
                    p.enter(RuleKind::CharPrimary);
                    cur = 83;
                }
                100 => { // neg_class:alt0:ret
                    cur = p.ret();
                }
                101 => { // neg_class:alt1:expect:LPAREN
                    p.expect(TokenKind::Lparen, SYNC_3, "expected LPAREN");
                    p.push_ret(103);
                    p.enter(RuleKind::CharPrimary);
                    cur = 83;
                }
                103 => { // neg_class:alt1:star
                    match p.look(0).kind {
                        Some(TokenKind::Pipe) => {
                            p.push_ret(103); cur = 106;
                        }
                        _ => {
                            cur = 104;
                        }
                    }
                }
                104 => { // neg_class:alt1:expect:RPAREN
                    p.expect(TokenKind::Rparen, SYNC_3, "expected RPAREN");
                    cur = p.ret();
                }
                106 => { // neg_class:alt1:star-body:expect:PIPE
                    p.expect(TokenKind::Pipe, SYNC_3, "expected PIPE");
                    p.push_ret(108);
                    p.enter(RuleKind::CharPrimary);
                    cur = 83;
                }
                108 => { // neg_class:alt1:star-body:ret
                    cur = p.ret();
                }
                _ => unreachable!("unknown state"),
            }
        }
        p.set_state(cur);
    }
}



/// Parse the `file` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_file_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_FILE)
}

/// Parse the `file` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_file_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_FILE)
}

/// Parse the `decl` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_decl_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_DECL)
}

/// Parse the `decl` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_decl_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_DECL)
}

/// Parse the `annots` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_annots_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_ANNOTS)
}

/// Parse the `annots` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_annots_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_ANNOTS)
}

/// Parse the `annot` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_annot_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_ANNOT)
}

/// Parse the `annot` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_annot_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_ANNOT)
}

/// Parse the `alt_expr` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_alt_expr_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_ALT_EXPR)
}

/// Parse the `alt_expr` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_alt_expr_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_ALT_EXPR)
}

/// Parse the `seq_expr` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_seq_expr_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_SEQ_EXPR)
}

/// Parse the `seq_expr` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_seq_expr_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_SEQ_EXPR)
}

/// Parse the `group` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_group_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_GROUP)
}

/// Parse the `group` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_group_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_GROUP)
}

/// Parse the `atom` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_atom_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_ATOM)
}

/// Parse the `atom` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_atom_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_ATOM)
}

/// Parse the `char_primary` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_char_primary_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_CHAR_PRIMARY)
}

/// Parse the `char_primary` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_char_primary_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_CHAR_PRIMARY)
}

/// Parse the `neg_class` rule from an in-memory string.
///
/// Zero-copy: tokens borrow their text from `src`.
pub fn parse_neg_class_from_str<'a>(src: &'a str) -> Parser<'a, Scanner<'a, TokenKind, LexerDfa>> {
    Parser::new(Scanner::new(src), ENTRY_NEG_CLASS)
}

/// Parse the `neg_class` rule from any [`Read`] source.
///
/// Streaming: tokens own their text; memory use stays bounded regardless of
/// input size.
pub fn parse_neg_class_from_reader<R: Read>(reader: R) -> Parser<'static, StreamingLexer<R, TokenKind, LexerDfa>> {
    Parser::new(StreamingLexer::new(reader), ENTRY_NEG_CLASS)
}

